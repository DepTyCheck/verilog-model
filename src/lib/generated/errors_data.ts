...
export const allFoundErrors: FoundError[] = [
  ...
  },
  {
    ...
    "examples": [
      {
        "id": "output_port_is_connected_to_constants_auto_hierarchy_execute_6",
        "first_found": "2025-07-21T00:00:00.000Z",
        "minified_example": null,
        "minified_error": null,
      ...
      {
        "id": "output_port_is_connected_to_constants_auto_hierarchy_number",
        "first_found": "2025-07-21T00:00:00.000Z",
        "minified_example": null,
        "minified_error": null,
      ...
      {
        "id": "output_port_is_connected_to_constants_auto_hierarchy_execute_8",
        "first_found": "2025-07-29T00:00:00.000Z",
        "minified_example": null,
        "minified_error": null,
        "full_error": "ERROR: Output port qxuelserbc.myq.o (faeccq) is connected to constants: { $auto$hierarchy.cc:1462:execute$8 1'0 }\n",
        "full_example": "module faeccq\n  (output bit [0:0][1:4] o, output realtime ydvwszkvj, input bit fhmhaog);\n\n\n  not zf(venmstbh, venmstbh);\n\n  not jkklhzea(vwdydcds, jfwfrk);\n\n  xor gerofmaezg(vpvhsrcou, fdltlua, ldvez);\n\n\n  // Top inputs -> top outputs assigns\n\n  // Single-driven assigns\n\n  // Multi-driven assigns\n  assign vpvhsrcou = 'b1;\n  assign vwdydcds = 'b1;\nendmodule: faeccq\n\nmodule zpyz\n  ( output wor logic [4:4] alsttgmta [0:3][1:0]\n  , output logic [0:2][4:2][0:0] nh\n  , output reg nqhz\n  , output tri1 logic [4:3][4:0] nhigcjgep\n  );\n\n\n\n  // Top inputs -> top outputs assigns\n\n  // Single-driven assigns\n  assign nh = 'b1;\n\n  // Multi-driven assigns\n  assign alsttgmta = '{'{'bx,'bz},'{'bx,'b1},'{'bz,'b0},'{'bx,'b1}};\n  assign nhigcjgep = 'bx;\n  assign nhigcjgep = 'bx;\nendmodule: zpyz\n\nmodule po\n  (input bit oiikoicqad [1:4]);\n\n\n  not fxrde(mxhzdcoaq, ukrslwcls);\n\n  faeccq vshgsghbx(.o(o), .ydvwszkvj(yzbuves), .fhmhaog(rbkbpumr));\n  // warning: implicit conversion of port connection truncates from 4 to 1 bits\n  // warning: implicit conversion changes possible bit states from 2-state to 4-state\n  //   bit [0:0][1:4] o -> wire logic o\n  //\n  // warning: implicit conversion of port connection truncates from 64 to 1 bits\n  // warning: implicit conversion changes signedness from signed to unsigned\n  //   realtime ydvwszkvj -> wire logic yzbuves\n  //\n  // warning: implicit conversion changes possible bit states from 4-state to 2-state\n  //   wire logic rbkbpumr -> bit fhmhaog\n\n  xor xuxzar(gohvszw, jxkizvjwy, vehutrsz);\n\n  not ixhj(xui, xui);\n\n\n  // Top inputs -> top outputs assigns\n\n  // Single-driven assigns\n  assign vehutrsz = 'bz;\n\n  // Multi-driven assigns\n  assign mxhzdcoaq = 'b1;\n  assign mxhzdcoaq = 'bz;\nendmodule: po\n\nmodule qxuelserbc\n  ( output bit [1:0][0:3][3:2] wpcsja\n  , output logic qdwwz\n  , output tri0 logic vy [4:1][4:3][4:0]\n  , output trior logic [1:0][2:2][4:0][3:2] phxsjkjj [4:0][2:3][4:0][2:3]\n  );\n\n\n  faeccq myq(.o(rd), .ydvwszkvj(qdwwz), .fhmhaog(qdwwz));\n  // warning: implicit conversion of port connection truncates from 4 to 1 bits\n  // warning: implicit conversion changes possible bit states from 2-state to 4-state\n  //   bit [0:0][1:4] o -> wire logic rd\n  //\n  // warning: implicit conversion of port connection truncates from 64 to 1 bits\n  // warning: implicit conversion changes signedness from signed to unsigned\n  // warning: implicit conversion changes possible bit states from 4-state to 2-state\n  //   realtime qdwwz -> bit fhmhaog\n\n\n  // Top inputs -> top outputs assigns\n\n  // Single-driven assigns\n  assign phxsjkjj = '{'{'{'{'b1,'bz},'{'bz,'b0},'{'b1,'b1},'{'b0,'b1},'{'b1,'bz}},'{'{'b1,'b1},'{'bx,'b0},'{'b0,'b0},'{'bx,'bx},'{'b1,'bx}}},'{'{'{'bx,'bz},'{'bz,'bz},'{'b1,'b1},'{'bz,'b0},'{'bx,'b1}},'{'{'bx,'b0},'{'bx,'b1},'{'bz,'b1},'{'bz,'b1},'{'bz,'b1}}},'{'{'{'b0,'bz},'{'b0,'bz},'{'bz,'b1},'{'bz,'b1},'{'bz,'b0}},'{'{'b1,'b0},'{'b0,'b0},'{'bx,'b1},'{'bz,'b0},'{'b0,'b0}}},'{'{'{'b0,'b0},'{'bz,'b0},'{'b0,'b1},'{'b1,'b0},'{'bz,'bx}},'{'{'bz,'b1},'{'b0,'bz},'{'bx,'b1},'{'bz,'bz},'{'bz,'b1}}},'{'{'{'bx,'bz},'{'bz,'bz},'{'bx,'bx},'{'bz,'b1},'{'b0,'bz}},'{'{'bz,'b1},'{'b0,'bx},'{'b1,'bz},'{'bx,'bx},'{'bz,'bz}}}};\n  assign wpcsja = 'b1;\n  assign vy = '{'{'{'b1,'b0,'bx,'bz,'bx},'{'bz,'bx,'b1,'b0,'bx}},'{'{'bz,'b1,'b1,'bz,'bz},'{'bx,'bx,'bz,'b1,'b0}},'{'{'b0,'bz,'bz,'bz,'b1},'{'bz,'bx,'bx,'b0,'b1}},'{'{'bx,'bx,'bx,'b1,'b1},'{'bz,'bz,'bz,'b1,'bz}}};\n\n  // Multi-driven assigns\n  assign rd = 'b0;\n  assign rd = 'b1;\n  assign phxsjkjj = '{'{'{'{'b0,'b1},'{'bx,'b0},'{'b0,'bx},'{'bx,'bz},'{'bz,'bz}},'{'{'bz,'bx},'{'bx,'b1},'{'bx,'bz},'{'b1,'b1},'{'bx,'bx}}},'{'{'{'b0,'b1},'{'bx,'bx},'{'bz,'b1},'{'bz,'bz},'{'bx,'b1}},'{'{'bz,'bx},'{'bz,'b0},'{'bz,'bx},'{'bz,'bz},'{'bx,'b1}}},'{'{'{'bz,'b1},'{'b0,'bz},'{'b0,'bz},'{'bz,'bx},'{'bx,'b0}},'{'{'b1,'b0},'{'b1,'b1},'{'bx,'bx},'{'bz,'bz},'{'bz,'b0}}},'{'{'{'bz,'b1},'{'bz,'bx},'{'bz,'b0},'{'b1,'b0},'{'bz,'b1}},'{'{'b0,'b0},'{'b0,'b0},'{'b1,'b0},'{'bz,'bz},'{'bx,'b0}}},'{'{'{'bz,'b0},'{'b1,'bz},'{'bx,'b0},'{'b0,'bx},'{'b1,'b0}},'{'{'bz,'bx},'{'b1,'b1},'{'bx,'b0},'{'bx,'bx},'{'bx,'b0}}}};\n  assign phxsjkjj = '{'{'{'{'bx,'bx},'{'bx,'b0},'{'bz,'b1},'{'b1,'b1},'{'bx,'bz}},'{'{'b0,'bz},'{'b1,'b0},'{'bx,'bx},'{'b0,'bx},'{'bz,'b0}}},'{'{'{'bz,'bz},'{'bz,'b1},'{'b0,'b0},'{'b0,'bz},'{'b0,'bz}},'{'{'b1,'bx},'{'b1,'b0},'{'bz,'b0},'{'bx,'bz},'{'b0,'b0}}},'{'{'{'bz,'bx},'{'b1,'bx},'{'bx,'b0},'{'bx,'b0},'{'b0,'bx}},'{'{'b0,'bz},'{'bx,'b0},'{'b1,'bx},'{'bz,'b1},'{'bz,'bz}}},'{'{'{'bx,'b0},'{'b1,'b0},'{'b0,'bz},'{'bz,'b1},'{'b1,'b1}},'{'{'bx,'bz},'{'b0,'bx},'{'b1,'b0},'{'b1,'b0},'{'bx,'b1}}},'{'{'{'bz,'bz},'{'bz,'bx},'{'bz,'b0},'{'bx,'b0},'{'b1,'b0}},'{'{'bx,'b0},'{'b1,'b1},'{'bz,'b0},'{'bz,'bx},'{'bx,'b1}}}};\nendmodule: qxuelserbc\n"
      }
    ],
    "issue_status": null,
    "issue_link": null
