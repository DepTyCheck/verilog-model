module Test.Common.Multiconnection
...
namespace DesignUnitSig
  ...
  ||| 14.7.2) or an out, inout, buffer, or linkage port...
  ||| IEEE 1076-2019
  |||
  ||| So Out, InOut, Buffer and Linkage are sources
  public export
  ...
  writePortModeVHDL Linkage = True
...
namespace VHDLRules
  ...
  ||| IEEE 1076-2019
  |||
  ||| Direction predicate for the 6.5.6.3 association rules.
  ||| This is distinct from `writePortModeVHDL`, which counts sources for 6.4.2.3 and treats linkage as a source.
  ||| Here only out/inout/buffer impose the "actual must be drivable" constraint;
  ||| in and linkage formals place no direction restriction
  ||| (rule e lets a linkage formal associate with an actual of any mode, and linkage actuals are handled separately by `checkLinkage`).
  public export
  drivingDirVHDL : VHDLPortMode -> Bool
  drivingDirVHDL In      = False
  drivingDirVHDL Out     = True
  drivingDirVHDL InOut   = True
  drivingDirVHDL Buffer  = True
  drivingDirVHDL Linkage = False

  -- The logic corresponds to material conditional (. . )
  public export
  checkDirectionVHDL : VHDLPortMode -> VHDLPortMode -> Bool
  checkDirectionVHDL formal actual with (drivingDirVHDL formal)
    checkDirectionVHDL formal actual | False = True -- formal is in or linkage
    checkDirectionVHDL formal actual | True  = case drivingDirVHDL actual of
