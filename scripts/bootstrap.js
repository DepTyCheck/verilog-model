...
function main() {
	...
	step(
		...
		() => {
			...
			run('./.venv/bin/pip', ['install', '-r', 'ci/runner/dataset_stats/requirements.txt'], { cwd: path.join(cwd, 'verilog-model') });
		}
	);

	step(
		'generate combined_stats.csv',
		...
		() =>
			run(
				...
				[
					...
					'../dataset/files',
					'--found-issues-dir',
					'../dataset/found_issues',
					'--legacy-stats-csv',
					'../dataset/legacy_stats.csv',
					'--output',
					'../combined_stats.csv'
				],
				{
