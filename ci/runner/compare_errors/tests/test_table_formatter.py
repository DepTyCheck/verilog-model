...
class TestTableFormatter(unittest.TestCase):
    def _make_deltas(self):
        return [
            ErrorPercentageDelta(
                "t_dll_api_cc_ivl_nexus_s", historical_pct=3.76, current_pct=32.0
            ),
            ...
            ErrorPercentageDelta(
                "stale_error_not_in_current_run", historical_pct=1.88, current_pct=0.0
            ),
        ]
    ...
    def test_positive_delta_has_plus_sign(self):
        table = format_table(
            [ErrorPercentageDelta("foo", historical_pct=5.0, current_pct=10.0)]
        )
        self.assertIn("+5.00%", table)

    def test_zero_delta_formatted(self):
        table = format_table(
            [ErrorPercentageDelta("bar", historical_pct=5.0, current_pct=5.0)]
        )
    ...
    def test_negative_delta_formatted(self):
        table = format_table(
            [ErrorPercentageDelta("baz", historical_pct=10.0, current_pct=7.0)]
        )
    ...
    def test_rows_appear_in_sorted_order_by_abs_delta(self):
        deltas = [
            ...
            ErrorPercentageDelta(
                "large_positive", historical_pct=0.0, current_pct=32.0
            ),
            ErrorPercentageDelta(
                "large_negative", historical_pct=20.0, current_pct=5.0
            ),
        ]
    ...
    def test_percentages_use_two_decimal_places(self):
        table = format_table(
            [ErrorPercentageDelta("foo", historical_pct=3.759398, current_pct=12.5)]
        )
