#!/usr/bin/env python3
"""
known_errors_check — regression checker for known tool bugs.

Usage (from ci/runner/):
  python -m known_errors_check.main \
    --known-errors-dir verilog-gh-pages/found_errors \
    --output known-errors-check-report.json \
    --tool-name iverilog \
...
from known_errors_check.src.parse_args import parse_args
...
def main() -> None:
    configure_logger(name="known_errors_check", level=logging.INFO)
